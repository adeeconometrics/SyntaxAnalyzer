...
auto Scanner::string() -> void {
  while (peek() != '"' && !is_at_end()) {
    if (peek() == '\n')
      m_line += 1;
    static_cast<void>(advance()); // look into why we need to convert char to void
  }
  ...
  const std::string string_literal = m_source.substr(m_start + 1, string_size - 2);
...
auto Scanner::add_token(const TokenType type, const std::string& token) -> void {
